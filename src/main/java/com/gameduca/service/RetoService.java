...
public class RetoService {
	...
	public List<AlumnoRetoDTO> obtenerRetosAlumno(Long idAsignatura) {
		...
		List<AlumnoReto> listaAlumnoReto = new ArrayList<>();
    	UserDetails usuario = (UserDetails) SecurityContextHolder.getContext().getAuthentication()
				.getPrincipal();
    	String rol = usuario.getAuthorities().iterator().next().getAuthority();
    	String nombreUsuario = usuario.getUsername();
    	if(rol.equals(RolNombre.ROLE_USER.name())) {
    		listaAlumnoReto = alumnoRetoRepository.findListAlumnoRetoByAlumnoyAsignatura(nombreUsuario, idAsignatura);
    	} else {
    		listaAlumnoReto = alumnoRetoRepository.findAlumnoRetoByAsignatura(idAsignatura);
    	}	
		listaAlumnoRetoDTO = alumnoRetoDTOMapper.toDTO(listaAlumnoReto);
