...
public interface AlumnoAsignaturaRepository extends CrudRepository<AlumnoAsignatura, Long> {
	...
	@Query("SELECT alumnoasignatura.alumno FROM AlumnoAsignatura alumnoasignatura where alumnoasignatura.asignatura.id =:id and alumnoasignatura.estado = 'PETICION_ENVIADA'")
